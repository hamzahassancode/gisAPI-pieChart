  ...
  require([
    ...
    view.whenLayerView(featureLayer).then((layerView) => {
      ...
      function applyFilter() {
        ...
        const optionType = fieldSelect.options[fieldSelect.selectedIndex].fieldType;

        if (inputValue !== "") {
          if (optionType === "integer" || optionType === "double") {
            featureLayer.definitionExpression = `${field} = ${inputValue}`;
          } else if (optionType === "string") {
            featureLayer.definitionExpression = `upper(${field}) LIKE '%${inputValue.toUpperCase()}%'`;
          } else {
            console.warn("Unsupported field type:", optionType);
          }
        } else {
          featureLayer.definitionExpression = "1=1";
        }

        tableFilter(inputValue, field);
    ...
    function tableFilter(inputValue, field) {
      const fields = ["AREA_ID", "POLYGON_NM", "Population"];
      let query = featureLayer.createQuery();

      if (inputValue !== "") {
        debugger;
        query.where = `${field} LIKE '%${inputValue}%'`.toLowerCase();
      } else {
    ...
    function pieChart() {
      ...
      .then(function(response) {
          const features = response.features;
          const pieChartData = features.map(feature => ({
              value: feature.attributes["TOTALCOUNT"],
              name: feature.attributes["UNIT_TYPE"]
          }));
